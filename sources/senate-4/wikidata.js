...
module.exports = function () {
  return `SELECT DISTINCT ?item ?itemLabel ?start ?end
    ...
    WHERE {
      ...
      FILTER (?startV < "${meta.term.end}T00:00:00Z"^^xsd:dateTime)
      FILTER (!BOUND(?endV) || ?endV > "${meta.term.start}T00:00:00Z"^^xsd:dateTime)
    ...
    ORDER BY ?position ?item ?start ?end ?ps`
}
