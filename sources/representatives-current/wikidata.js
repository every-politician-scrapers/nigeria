...
module.exports = function () {
  ...
  return `SELECT DISTINCT ?item ?name ?party ?area
    ...
    WITH {
      ...
      WHERE {
          ...
          FILTER NOT EXISTS { ?ps pqv:P582 ?p39end }
          OPTIONAL {
            ?ps pq:P2937 ?term .
    ...
    WHERE {
      ...
      OPTIONAL {
        ?ps pq:P768 ?areaItem .
        OPTIONAL { ?areaItem rdfs:label ?area FILTER (LANG(?area)="en") }
      }

      OPTIONAL {
        ?ps prov:wasDerivedFrom ?ref .
        ?ref pr:P854 ?source FILTER CONTAINS(STR(?source), 'nass.gov.ng').
